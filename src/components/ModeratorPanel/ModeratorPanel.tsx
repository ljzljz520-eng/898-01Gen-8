...
import { Eye, EyeOff, Star, FileText, AlertTriangle, X, Check } from 'lucide-react';
...
export default function ModeratorPanel({ 
  ...
  const { hidePost, unhidePost, markAsFeatured, unmarkAsFeatured, requireSupplement: requestSupplement } = usePostStore();
  ...
  const handleHide = () => {
    ...
    hidePost(postId, hideReason.trim());
  ...
  const handleToggleFeatured = () => {
    if (isFeatured) {
      unmarkAsFeatured(postId);
    } else {
      markAsFeatured(postId);
    }
  };
  ...
  const handleRequestSupplement = () => {
    ...
    requestSupplement(postId, supplementMessage.trim());
      ...
      <div className="metal-card border-warning-orange-500/30">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          ...
          <button
            ...
            className="px-4 py-3 rounded-lg font-medium transition-all flex flex-col items-center gap-1 bg-warning-orange-500/20 text-warning-orange-400 border border-warning-orange-500/50 hover:bg-warning-orange-500/30"
          >
